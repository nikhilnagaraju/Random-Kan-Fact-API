...
const { normalizeCount, getRandomFacts, getFactByID, createFact, removeFactByID, updateFactByID } = require('./utils');
...
router.get('/random', async (req, res) => {
  try{
    ...
    console.error(err);
    res.status(400).json({ status: 'failure', error: 'An error has occurred while fetching data' });
  }
});
