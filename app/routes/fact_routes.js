module.exports = function(app, databaseObj){
  ...
  app.get('/random', (req, res) => {
      const getid = '' + getRandomIntInclusive(1, 260);
      ...
      db.collection('factslist').findOne(details, (err, item) => {
        if(err){
          ...
          if(item){ res.send(item); }else{ res.send({ status: 'undetermined', error: 'An error has occurred while fetching data' }); }
        }
      });
  });
  ...
  app.get('/facts', (req, res) => {
      const objt = {};
        db.collection('factslist').aggregate([{ $match: {} }, { $sample: { size: 10 } }]).toArray((err, item) => {
          if(err){
